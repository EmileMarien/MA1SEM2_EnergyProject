...
def electricity_cost(solar_panel_count: int=1, panel_surface:int= 1.6 ,annual_degredation: float=0.02, panel_efficiency: int= 0.55, temperature_coefficient: float=-0.0026, inverter_size_AC: int = 5, inverter_maxsolar_DC: int = 5, inveter_maxbattery_DC: int=5,tilt_angle:int=-1, Orientation:str="S", battery_capacity: float= 1000, battery_count: int=1, data_management_cost: float =53.86, capacity_rate: float=41.3087, tariff: str='DualTariff', purchase_rate_injection: float=0.0041125, purchase_rate_consumption: float=0.0538613, energy_contribution_levy: float=8.72, energy_fund_contribution_levy: float=0, special_excise_duty_levy: float=215.16):
    """
    ...
    irradiance.power_flow(max_charge=battery_capacity*battery_count, max_AC_power_ouput = inverter_size_AC, max_PV_input = inverter_maxsolar_DC, max_DC_batterypower = inveter_maxbattery_DC)
